    ...
    if (isSupSearch) {
      ...
      dataCards.forEach (data => {
        ...
        tableIngredients = data.ingredients
          ...
          .join (' ');
        if (
          data.name.toLowerCase ().includes (dataInput) ||
          data.description.toLowerCase ().includes (dataInput) ||
          tableIngredients.toLowerCase ().includes (dataInput)
        ) {
          vCard.push (data);
        }
      });
...
export const filterByIngredient = ingredients => {
  ...
  function executeSearch (data) {
    ...
    data.forEach (dataCard => {
      let tableIngredients = '';
      tableIngredients = dataCard.ingredients
        .map (ingredient => {
          return ingredient.ingredient;
        })
        .join (' ');
      ...
      ingredients.forEach (ingredient => {
        if (tableIngredients.toLowerCase ().includes (ingredient)) {
          // tableIngredients correspond aux ingredients de chaque recette.
          countIngredient++;
        }
      });
...
export const filterByUstensils = ustensils => {
  // ingredients correspond à la tableList
  ...
  function executeSearch (data) {
    ...
    data.forEach (dataCard => {
      let tableUstensils = '';
      tableUstensils = dataCard.ustensils
        .map (ustensil => {
          return ustensil;
        })
        .join (' ');
      ...
      ustensils.forEach (ustensil => {
        if (tableUstensils.toLowerCase ().includes (ustensil)) {
          // tableIngredients correspond aux ingredients de chaque recette.
          countUstensil++;
        }
      });
...
export const filterByAppareils = appareil => {
  // ingredients correspond à la tableList
  function executeSearch (data) {
    let vCard = [];
    data.forEach (dataCard => {
      ...
      if (tableAppareils.toLowerCase ().includes (appareil)) {
        // tableIngredients correspond aux ingredients de chaque recette.
        vCard.push (dataCard); // récupère toutes les recettes qui contiennes tous les ingredients choisi par l'utilisateur
      }
      // si le nombre d'ingredient trouvé est égale au nombre d'ingredient de la recette, alors on ajoute la recette dans le tableau vCard
