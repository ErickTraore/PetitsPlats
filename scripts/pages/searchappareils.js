...
function executeClick (data) {
  ...
  const listContent = document.querySelectorAll (
    '.card__header__todolist__content'
  );
  ...
  btnsArray.forEach ((item, index) => {
    ...
    item.addEventListener ('click', function () {
      ...
      listContentArray.forEach ((list, index) => {
        //Boucle pour trouver l'élement à effacer.
        ...
        if (`${index}` === dataDelete) {
          ...
          const buttons = document.querySelectorAll (
            // on doit rechercher itemDelete dans la liste générale des ustensils.
...
const icon = document.querySelector (
  '.card__header__naving__columnTwo__header__title'
);
const activeDisplay = document.querySelector (
  '.card__header__naving__columnTwo__header__modal'
);
const iconup = document.querySelector (
  '.card__header__naving__columnTwo__header__title__img'
);
icon.addEventListener ('click', function () {
  activeDisplay.classList.contains ('active')
...
export function reqInputAppareil (vCardData) {
  ...
  inputAppareils.addEventListener ('input', event => {
    ...
    if (dataInput) {
      console.log (
        "insertion d'un élément dans la recherche(input) des appareils"
      );
      const activeHtml = document.getElementById ('appareilRemove');
      activeHtml.classList.contains ('active')
  ...
  const todoListModal = document.querySelectorAll (
    '.card__header__naving__columnTwo__header__modal__list__button'
  );
  ...
  if (inputAppareils) {
    inputAppareils.addEventListener ('input', event => {
      ...
      if (dataInput) {
        ...
        console.log (
          "insertion d'un élément dans la recherche(input) de l'appareil"
        );
      ...
      let cardAppareils = document.querySelector (
        '.card__header__naving__columnTwo__header__modal__list'
      );
