function cardFactory (data) {
  ...
  function makeCardDOM () {
    ...
    card8html.classList.add (
      'card__all__recette__body__content__ingredients__p'
    );
    card8html.textContent = 'Ingredients';
    card7html.appendChild (card8html);

    const card71html = document.createElement ('div');
    card71html.classList.add (
      'card__all__recette__body__content__ingredients__items'
    );
    ...
    function creerDivIngredient () {
      ingredients.forEach (data => {
        ...
        divxhtml.classList.add (
          'card__all__recette__body__content__ingredients__items__boucle'
        );
        card71html.appendChild (divxhtml);

        const div6html = document.createElement ('div');
        div6html.classList.add (
          'card__all__recette__body__content__ingredients__items__boucle__title'
        );
        divxhtml.appendChild (div6html);

        const p6html = document.createElement ('p');
        p6html.classList.add (
          'card__all__recette__body__content__ingredients__items__boucle__title__p'
        );
        p6html.textContent = data.ingredient;
        div6html.appendChild (p6html);

        const div7html = document.createElement ('div');
        div7html.classList.add (
          'card__all__recette__body__content__ingredients__items__boucle__quantity'
        );
        divxhtml.appendChild (div7html);

        const p7html = document.createElement ('p');
        p7html.classList.add (
          'card__all__recette__body__content__ingredients__items__boucle__quantity__p'
        );
